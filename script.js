...
(function() {
    ...
    // Muat turun poster wallpaper (grid + info, tanpa dropdown)
    window.muatTurunGambar = async function() {
        ...
        try {
            ...
            const scaleX = ww / srcW;
            const scaleY = wh / srcH;
            ...
            link.href = wallpaper.toDataURL('image/jpeg', 0.92);
            link.click();
        } catch(e) {
            console.error(e);
            hide.forEach((el, i) => { if (el) el.style.display = saved[i]; });
