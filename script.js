...
(function() {
    ...
    // Kongsi WhatsApp (gunakan data hijrah dari API jika ada)
    window.kongsiWhatsApp = async function() {
        ...
        // Dapatkan hijri dari API kalendar
        let hijriText = today.hijri || '';
        if (!hijriText || hijriText === 'TIDAK TERSEDIA') {
            const hijriData = await loadHijriData();
            const entry = hijriData?.find(item => item.gregorian === todayStr);
            if (entry?.hijri?.formatted) hijriText = entry.hijri.formatted;
        }
        
        const get12h = (obj) => obj?.['12h'] || '--:--';
        let w = today.waktu;
        let syuruk = w.syuruk || w.syuruq;
        let dhuha = w.dhuha;
        let isyak = w.isyak || w.isya;
        let mesej = `🕌 *Waktu Solat Malaysia* 🕌\n📍 ${lokasi}\n📅 ${tarikh}\n🌟 ${hijriText}\n\n`;
        mesej += `Imsak: ${get12h(w.imsak)}\nSubuh: ${get12h(w.subuh)}\nSyuruk: ${get12h(syuruk)}\n`;
        mesej += `Dhuha: ${dhuha ? get12h(dhuha) : '--:--'}\nZohor: ${get12h(w.zohor)}\nAsar: ${get12h(w.asar)}\n`;
        mesej += `Maghrib: ${get12h(w.maghrib)}\nIsyak: ${get12h(isyak)}\n\n`;
